...
test_tni <- function(){
  ...
  checkTrue(is.matrix(res) && ncol(res)==5)
  #tni.bootstrap
  ...
  checkTrue(is.matrix(res) && ncol(res)==5)
  #tni.conditional
  rtni<-tni.conditional(rtni, modulators=tfs4test, minRegulonSize=1)
  res<-tni.get(rtni,what="cdt")
  checkTrue(is.list(res) && length(res)==5)
  #tni.graph
  res<-tni.graph(rtni, gtype="mmap", tfs=tfs4test)
...
test_tna <- function(){
  ...
  checkTrue(is.data.frame(res) && ncol(res)==9)
  #tna.graph
  res<-tna.graph(rtna, tnet="ref", gtype="amap", tfs=tfs4test)
  checkTrue(is.igraph(res))
}
